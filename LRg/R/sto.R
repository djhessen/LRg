...
sto <- function(formula, q=NULL, data, tol=1.0e-8, maxit=100) {
  ...
  k <- dim(X)[2]-1
  if (is.ordered(model.response(mf))) {
    y <- as.integer(model.response(mf))-1
  } else if (min(model.response(mf))==1) {
    y <- as.integer(model.response(mf))-1
  } else {
  ...
  res <- round(cbind(x,se,z,1-pnorm(abs(z))),3)
  ...
  #    alpha=x[1:m],Beta=Beta,Gamma=Gamma,Phi=Phi,loglik=loglik,
  #              npars=npars,AIC=aic,fitted.values=fitv,residuals=res,pred.cat=pcat,
  #              obs.cat=y,CS=CS,NK=NK,MF=MF)
  } else if (q==0) {
    out <- list(pars=x[1:m],npars=npars)
