...
#'
#' This function can be used to fit the parallel adjacent-categories logit model.
#'
...
#' @export
pac <- function(formula, data, maxit=100) {
  ...
  for (ii in 1:maxit) {
    it <- it+1
    z <- x
    theta <- rbind(x[1:m],x[(m+1):(m+k)]%*%t(1:m))
    eta <- X%*%theta
  ...
  rownames(C) <- c(paste('(Intercept)',1:m,sep=':'),colnames(X[,2:(k+1)]))
